...
def Moon():  # d0 = first perihelion , d1 = today , d2 = next perihelion
    with open("/var/www/html/Orbit.json", "r") as O:
        ...
        for i in thisYear:
            ...
            if (
                d0 > d1 - timedelta(days=27) and d0 < d1
...
def Venus():  # d0 = first perihelion , d1 = today , d2 = next perihelion
    ...
    with open("/var/www/html/Orbit.json", "r") as O:
        orbit = json.load(O)
        thisYear = orbit["Venus"]  # This year
...
def Faye():  # d0 = first perihelion , d1 = today , d2 = next perihelion
    ...
    with open("/var/www/html/Orbit.json", "r") as O:
        orbit = json.load(O)
        thisYear = orbit["Faye"]  # This year
        # years_ago = orbit["Faye"][str(years_ago)][-1]
        # years_after = orbit["Faye"][str(years_after)][0]
        for i in thisYear:
            d0Year = i[:4]
            d0Year = int(d0Year)
            d0Month = i[5:7]
        ...
        for i in thisYear:
            ...
            if (
                ...
                d3 = Percentage.NewFayePerihelion - Percentage.FayePerihelion
                d3 = str(d3)
                d3 = d3.split()
                d3 = int(d3[0])
                d3 = d3 + 1
                ValuePercent = d3 / 100
                delta = d1 - Percentage.FayePerihelion
                delta = str(delta)
                delta = delta.split()
                delta = delta[0]
                new = re.sub("[^0-9]", "", str(delta))
                ...
                print("Comet : Faye")
                print(("Day of the year : ") + str("Day ") + str(new))
                print(("Year progress : ") + str(Percentage.FayeResult) + str("%"))
...
contentHtml.write("<br />")
contentHtml.write(
    '<img src="http://astrometry.ch/pictures/faye.jpg" alt="faye" width="100" height="100">'
)
contentHtml.write("<br />")
