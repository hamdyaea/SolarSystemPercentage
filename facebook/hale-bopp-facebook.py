...
def HaleBopp():  # d0 = first perihelion , d1 = today , d2 = next perihelion
    ...
    with open("/var/www/html/Orbit.json", "r") as O:
        ...
        # years_ago = orbit["HaleBopp"][str(years_ago)][-1]
        # years_after = orbit["HaleBopp"][str(years_after)][0]
        for i in thisYear:
            d0Year = i[:4]
            d0Year = int(d0Year)
            d0Month = i[5:7]
...
graph.put_photo(image=open("/var/www/html/pictures/hale-bopp.jpg", 'rb'), message=Percentage.HaleBoppHTML + Percentage.barrHaleBoppHTML + str('\n#Astronomy #Space #Espace #Astrometry'))